"""Uebertraegt den Datenbestand aus der SQLite-Datei nach PostgreSQL.

Aufruf im laufenden Container (DATABASE_URL muss auf das ZIEL zeigen, der
SQLite-Pfad kommt wie gewohnt aus DB_PATH):
...
from .database import IS_SQLITE, Base, engine, init_db
...
def main() -> int:
    ...
    if IS_SQLITE:
        logger.error(
            "DATABASE_URL zeigt nicht auf eine PostgreSQL-Datenbank (aktuell: %s). "
            "Das Ziel der Migration muss ueber DATABASE_URL gesetzt sein.",
            settings.database_url,
        )
        return 1
    if not settings.db_path.is_file():
        logger.error("Quelldatenbank %s nicht gefunden.", settings.db_path)
        return 1

    logger.info("Quelle: %s", settings.db_path)
    logger.info("Ziel:   %s", engine.url.render_as_string(hide_password=True))

    # Fehlende Tabellen im Ziel anlegen (und die kleinen Schema-Nachtraege
    # ausfuehren) - genau dieselbe Routine wie beim normalen App-Start.
    init_db()
    ...
    source = create_engine(
        f"sqlite:///file:{settings.db_path}?mode=ro&uri=true",

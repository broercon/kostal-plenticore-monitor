...
Verschickt einmal am Tag (konfigurierbare Uhrzeit, siehe
config.DAILY_REPORT_TIME) eine Mail mit einem Schnappschuss der Anlage –
welche Wechselrichter aktiv/erreichbar waren und wie viel PV-Ertrag sie
(einzeln + in Summe) an diesem Tag bereits erzielt haben – an
DAILY_REPORT_RECIPIENTS, verschickt über den zentralen Mail-Service (siehe
broercon/Mailserver, MAIL_SERVICE_URL).

Lässt sich zusätzlich manuell über POST /api/admin/daily-report/trigger
anstoßen (z.B. um die Konfiguration zu testen, ohne bis zur eingestellten
Uhrzeit zu warten) – GET /api/admin/daily-report/status zeigt den Stand des
letzten Versands/Fehlers, ohne dass man dafür die Container-Logs
durchsuchen müsste (gleiches Muster wie auto_import.get_import_status).
"""
...
async def generate_and_send_daily_report() -> dict:
    ...
    Trigger-Endpoint. Fängt JEDEN Fehler ab (Konfigurationsfehler,
    Mail-Service nicht erreichbar o.ä.) und merkt ihn sich in _state, statt
    ihn weiterzureichen – ein fehlgeschlagener Report darf den
    Scheduler-Loop nicht beenden."""
    ...
    try:
        ...
        await send_report_mail(subject, body)
    ...
    message = f"Report an {', '.join(settings.daily_report_recipients)} verschickt."
    logger.info(message)
...
class DailyReportScheduler:
    ...
    def start(self) -> None:
        if not settings.daily_report_enabled:
            logger.info("Täglicher Mail-Report deaktiviert (DAILY_REPORT_ENABLED=false).")
            return
        if not settings.daily_report_recipients or not settings.mail_service_url:
            logger.warning(
                "Täglicher Mail-Report nicht gestartet: DAILY_REPORT_RECIPIENTS "
                "und/oder MAIL_SERVICE_URL fehlen."
            )
            return
        self._stop_event = asyncio.Event()
        self._task = asyncio.create_task(self._run())
        logger.info(
            "Täglicher Mail-Report aktiv: %s Uhr (%s) an %s.",
            settings.daily_report_time,
            settings.timezone_name,
            ", ".join(settings.daily_report_recipients),
        )
    ...
    async def _run(self) -> None:
        while not self._stop_event.is_set():
            ...
            try:
                ...
                pass  # Zielzeit erreicht - jetzt Report verschicken
